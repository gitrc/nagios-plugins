...
my @targets = ( '1.3.6.1.2.1.25.5.1.1.2','1.3.6.1.2.1.25.2.2' );
...
foreach my $target (@targets) {
    ...
    if ( $s->version == SNMP_VERSION_1 ) {
        ...
        while ( defined( $s->get_next_request(@args) ) ) {
            ...
            if ( $target eq '1.3.6.1.2.1.25.5.1.1.2' ) {
                push @memstatus, $s->var_bind_list()->{$oid};
            }
            else {
                $totalmem = $s->var_bind_list()->{$oid};
            }
...
my $memfree = ($totalmem - $sum);
print
"MEM $STATUS: $sum kBytes utilized - $memfree kBytes free | mem=$sum\n";
